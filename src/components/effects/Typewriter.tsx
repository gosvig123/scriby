...
function TypewriterComponent({ baseText, rotatingText = [] }: TypewriterProps) {
  const [displayedBaseText, setDisplayedBaseText] = useState("");
  const [displayedRotatingText, setDisplayedRotatingText] = useState("");
    ...
    if (rotatingText.length > 0) {
      const timer = setTimeout(() => {
        if (!baseTextWritten) {
          ...
          const currentWord = rotatingText[currentIndex % rotatingText.length];

          if (isDeleting) {
            setDisplayedRotatingText(
              currentWord.substring(0, displayedRotatingText.length - 1)
            );
